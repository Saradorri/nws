...
func (rcl *RabbitClient) connect(isRec, reconnect bool) (*amqp.Connection, error) {
	...
	var c string
	if config.CNF.RabbitMQ.Username == "" {
		c = fmt.Sprintf("amqp://%s:%s/", config.CNF.RabbitMQ.Host, config.CNF.RabbitMQ.Port)
	} else {
		c = fmt.Sprintf("amqp://%s:%s@%s:%s/", config.CNF.RabbitMQ.Username, config.CNF.RabbitMQ.Password, config.CNF.RabbitMQ.Host, config.CNF.RabbitMQ.Port)
	}
...
func (rcl *RabbitClient) Consume(n string, f func(delivery amqp.Delivery, q string) error) {
	for {
		for {
